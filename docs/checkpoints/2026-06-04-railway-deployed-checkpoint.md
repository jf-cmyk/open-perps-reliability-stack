...
- Implemented `#17` Solana runtime failure taxonomy expansion with typed reason codes for account-state mismatch, invalid account sets, compute budget exhaustion, blockhash expiry, account-lock contention, priority-fee underbid, and unknown/dropped transactions.
- Added `scripts/discover_readonly_targets.py` for `#16`; first live Helius run reached the provider but returned `Unauthorized`, so the local `.env` Helius endpoint/key needs correction before live read-only proof output can be generated.
...
1. Keep adapting grant proposal language as the Railway MVP hardens.
2. Add hosted uptime or scheduled smoke monitoring from `#14`.
