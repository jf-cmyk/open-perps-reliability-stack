...
- Expanded the non-served invalid public-package fixture corpus from 6 to 14 cases:
...
- Data: expand invalid corpus once before grant submission, then stop around 12-15 focused cases.
- Liquidator/SDK: current Jupiter source-authority reason-code coverage is sufficient; tx-plan guardrail negatives are the next replay safety target.
...
1. Add tx-plan guardrail negative fixtures for signer/submission/execution-resource regressions.
2. Add bounded JSON Schema validation for positive package payload shape.
3. Continue Jupiter `PositionRequest` canonical authority diligence or request direct Jupiter confirmation.
4. If Jupiter remains blocked, start a Phoenix source-backed adapter/proof-package spike.
5. Refresh the local Word proposal from the improved markdown and grant-safe wording.
