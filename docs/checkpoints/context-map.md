...
10. Create the private Slack webhook and set `OPRS_ALERT_WEBHOOK_URL` with stdin, then test only the checked-in Slack sample payload through `railway run --service oprs-readonly-worker --no-local -- scripts/send_slack_alert_sample.py --send`.
11. Pick the first hosted worker command, schedule, and retention policy before deploying source to `oprs-readonly-worker`.
