...
| Drift public guardrails can be packaged as scrubbed public data. | `schemas/datasets/guardrail-snapshot-v0.json`, `examples/public/drift-guardrails-v0/`. | `scripts/validate_public_guardrail_package.py` validates checksums, row counts, readiness flags, and public-data scrub gates. |
| Drift is first, Jupiter Perps is second, and Phoenix/Rise is third. | `docs/protocol-targets.md` and decision log. | Manual review of `docs/protocol-targets.md`; Drift public identity, selected perp/spot metadata and guardrail decode, Jupiter target discovery, and Jupiter candidate pairing are live, while Drift market-economics decode and verified Jupiter request/fulfillment pairing remain pending. |
