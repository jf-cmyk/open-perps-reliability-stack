...
Local Helius access is confirmed for read-only target discovery. The first command probes the Drift program account and records follow-on venue lanes. The deeper Drift command derives public Drift PDAs from pinned official SDK source and confirms metadata access for the Drift state account, selected perp/spot market accounts, and selected oracle accounts. Its optional shape snapshot mode confirms Drift state/perp/spot account discriminator matches, data lengths, and account-data hashes without committing raw bytes or claiming field decode. The Jupiter command resolves current official-doc program/custody/oracle targets and confirms metadata access. All commands write scrubbed live output under `target/` and do not print the RPC URL. Drift decoder/IDL provenance is pinned; Jupiter has a docs-linked IDL candidate but not canonical decode authority yet. The next blockers are Drift public-field decode implementation, Jupiter canonical IDL/source confirmation, and Jupiter request/fulfillment transaction-history proof.
