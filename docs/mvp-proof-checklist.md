...
| Drift is first, Jupiter Perps is second, and Phoenix/Rise is third. | `docs/protocol-targets.md` and decision log. | Manual review of `docs/protocol-targets.md`; Drift and Jupiter target discovery are live, while binary decode and Jupiter request/fulfillment proof remain pending. |
...
Local Helius access is confirmed for read-only target discovery. The first command probes the Drift program account and records follow-on venue lanes. The deeper Drift command derives public Drift PDAs from pinned official SDK source and confirms metadata access for the Drift state account, selected perp/spot market accounts, and selected oracle accounts. The Jupiter command resolves current official-doc program/custody/oracle targets and confirms metadata access. All commands write scrubbed live output under `target/` and do not print the RPC URL. Drift decoder/IDL provenance is pinned; Jupiter has a docs-linked IDL candidate but not canonical decode authority yet. The next blockers are binary decode implementation, Jupiter canonical IDL/source confirmation, and Jupiter request/fulfillment transaction-history proof.
