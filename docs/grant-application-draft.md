...
- Public proof-package contracts for Drift guardrails, Jupiter onchain account-layout decode, Jupiter lifecycle/replay gaps, Phoenix/Rise market telemetry readiness, slot-regime benchmark windows, and Phoenix/Hawkeye validator-plan gates.
...
Source-backed Solana ecosystem context strengthens the need for this work without changing the MVP scope: Solana's June 2026 WSOP and MoneyGram/SDP announcements point to real-money payment and institutional infrastructure use cases, while Solana's bare-metal validator-readiness guidance, July 2026 100M-CU block-capacity activation, and August 2026 400ms-to-350ms slot-time activation highlight how execution reliability depends on validator, RPC, compute, slot timing, replay, catchup, and block-propagation conditions. OPRS stays focused on read-only open perps developer tooling in the grant phase; these runtime regimes are benchmark context, not claimed performance improvements.
...
- Helius target discovery now succeeds for Drift program/state/selected market/selected oracle metadata and Jupiter Perps program/custody/oracle metadata. Drift selected public guardrail fields are source-backed and package-validated, and Drift legacy liquidation-history diligence has scanned 308,000 finalized transactions from July 22 back through slot 415272972 at 2026-04-24T04:25:58Z without a matching `Liquidate*` log. That scan is bounded queue progress only, not evidence that liquidations were absent. Drift market-economics decode and historical replay still need source-backed proof before they can be claimed.
- Jupiter Perps now has a hash-pinned onchain Anchor IDL and scrubbed `Position` / `PositionRequest` account-layout decode package. Verified request/fulfillment reconstruction, keeper behavior, and replay readiness remain blocked pending lifecycle role maps or public fixture signatures.
