...
Solana's June 2026 validator-infrastructure guidance and the July 2026 mainnet activation of the 100M-CU block-capacity regime add useful ecosystem context: as compute limits rise and validator/network requirements become more explicit, read-only reliability tooling needs to explain how oracle freshness, account locks, decode quality, RPC behavior, market-data integrity, replay, and catchup behavior interact with the underlying execution environment. This paragraph is context only; the larger block-capacity regime is a benchmark context, not a measured performance claim, and validator telemetry, XDP tuning, block-propagation optimization, and private infrastructure operations are not grant MVP deliverables.
...
The Helius integration is limited to read-only RPC account fetches for decode/provenance proof. RPC URLs and API keys remain local-only and are never committed to the repo or included in public datasets. Drift program/state/selected market/selected oracle metadata discovery now succeeds, selected Drift perp and spot public guardrail fields are decoded with pinned offsets, and bounded Drift liquidation-history pagination has advanced to 266,000 finalized transactions without a candidate. Jupiter Perps program/custody/oracle metadata discovery plus unverified stronger lifecycle candidate labeling now succeed. Public Jupiter proof now includes three rejected blocker fixtures: stronger unverified lifecycle, weak lifecycle, and malformed source authority. Phoenix/Rise is now source-pinned for production program and Hawkeye view planning, with a local validator-plan contract for scrubbed fixture gates. Jupiter canonical IDL/source confirmation, verified request/fulfillment reconstruction, Phoenix account-level decode, and Phoenix liquidation replay remain next implementation steps only after source-review validators and scrub checks pass.
