...
Open Perps Reliability Stack is open-source developer tooling for Solana onchain perps reliability. It provides read-only protocol adapters, Pyth-aware risk primitives, normalized market-quality data, fixture-backed deterministic dry-run replay, and public dashboard/API schemas.
...
Solana's June 2026 validator-infrastructure guidance, the July 2026 mainnet activation of the 100M-CU block-capacity regime, and the August 2026 400ms-to-350ms slot-time activation add useful ecosystem context: as compute and slot regimes change and validator/network requirements become more explicit, read-only reliability tooling needs to explain how oracle freshness, account locks, decode quality, RPC behavior, market-data integrity, replay, and catchup behavior interact with the underlying execution environment. This paragraph is context only; these runtime regime boundaries are benchmark context, not measured performance claims, and validator telemetry, XDP tuning, block-propagation optimization, and private infrastructure operations are not grant MVP deliverables.
...
- Read-only target discovery and source-authority commands for Helius-backed proof setup. The commands are local-only or public HTTPS only, write scrubbed output under `target/`, confirm credentials are not printed, and have successfully read Drift public program/state/selected market/selected oracle metadata plus Jupiter Perps public program/custody/oracle metadata. Drift now emits selected public perp/spot identity, oracle identity, metadata, and guardrail fields. Drift legacy liquidation-history diligence has scanned 328,000 finalized program transactions from July 22 back through slot 414754843 at 2026-04-21T19:23:41Z without a matching `Liquidate*` log; this is bounded queue progress only, not evidence that liquidations were absent. Jupiter now emits stronger unverified lifecycle candidates when shared Jupiter-owned non-executable accounts are observed, includes malformed source-authority rejection coverage, has a hash-pinned onchain Anchor IDL plus scrubbed `Position` / `PositionRequest` account-layout decode proof, and adds a local-only role-map probe that binds sampled transaction accounts to onchain-IDL instruction roles while publishing only hashed summaries under `target/`. Verified Jupiter request/fulfillment pairing and replay remain blocked. Phoenix/Rise production program and Hawkeye view constants are pinned from Ellipsis Labs Rise source for read-only planning, and a Phoenix/Hawkeye validator-plan contract now records the next local gates. A new slot-regime benchmark package records the source-governed 400ms-to-350ms activation boundary for future read-only normalization without performance claims. Account-level Phoenix decode, trader monitoring, oracle-input identity, liquidation replay, and historical reconstruction are not claimed.
...
- Public contract index with package-level claim boundaries for Drift guardrail, Jupiter onchain decode, Jupiter lifecycle/replay authority-gap, Phoenix/Rise market-telemetry, and Phoenix/Hawkeye validator-plan packages.
- Eleven synthetic replay scenarios: eight Drift cases plus three Jupiter blocker fixtures. The Jupiter fixtures are intentionally rejected with `AdapterDecodeFailed`, `DataQualityLow`, and `ExecutionDisabledDryRun` because source authority and request/fulfillment pairing remain unverified; the malformed source-authority fixture also carries `AdapterVersionMismatch`.
- Public Jupiter onchain decode package that pins the live Anchor IDL and publishes scrubbed `Position` / `PositionRequest` account-layout evidence.
- Public Jupiter authority-gap package that explains which account-role, pairing, and replay prerequisites still block verified Jupiter lifecycle claims.
...
The Helius integration is limited to read-only RPC account fetches for decode/provenance proof. RPC URLs and API keys remain local-only and are never committed to the repo or included in public datasets. Drift program/state/selected market/selected oracle metadata discovery now succeeds, selected Drift perp and spot public guardrail fields are decoded with pinned offsets, and bounded Drift liquidation-history pagination has advanced to 328,000 finalized transactions without a candidate. Jupiter Perps program/custody/oracle metadata discovery plus unverified stronger lifecycle candidate labeling now succeed, the live onchain Anchor IDL now supports scrubbed `Position` / `PositionRequest` account-layout decode, and the new local role-map probe turns sampled public transactions into hashed instruction-role observations. Public Jupiter proof now includes the onchain decode package plus three rejected blocker fixtures: stronger unverified lifecycle, weak lifecycle, and malformed source authority. Phoenix/Rise is now source-pinned for production program and Hawkeye view planning, with a local validator-plan contract for scrubbed fixture gates. The slot-regime benchmark package adds source-governed pre/post windows around activation slot 440208000 while explicitly blocking performance claims. Verified Jupiter request/fulfillment reconstruction, Phoenix account-level decode, and Phoenix liquidation replay remain next implementation steps only after source-review validators and scrub checks pass.
