...
- Read-only target discovery and source-authority commands for Helius-backed proof setup. The commands are local-only or public HTTPS only, write scrubbed output under `target/`, confirm credentials are not printed, and have successfully read Drift public program/state/selected market/selected oracle metadata plus Jupiter Perps public program/custody/oracle metadata. Drift now emits selected public perp/spot identity, oracle identity, metadata, and guardrail fields. Drift legacy liquidation-history diligence has scanned 140,000 finalized program transactions from July 22 back through slot 418377065 on May 8 without a matching `Liquidate*` log; this is bounded queue progress only, not evidence that liquidations were absent. Jupiter now emits stronger unverified lifecycle candidates when shared Jupiter-owned non-executable accounts are observed, and includes malformed source-authority rejection coverage, while its source-authority audit keeps binary decode blocked until a canonical IDL/source is confirmed. Phoenix/Rise production program and Hawkeye view constants are pinned from Ellipsis Labs Rise source for read-only planning, but account-level decode, trader monitoring, oracle-input identity, liquidation replay, and historical reconstruction are not claimed.
...
- Public contract index with package-level claim boundaries for Drift guardrail, Jupiter authority-gap, and Phoenix/Rise market-telemetry packages.
...
The Helius integration is limited to read-only RPC account fetches for decode/provenance proof. RPC URLs and API keys remain local-only and are never committed to the repo or included in public datasets. Drift program/state/selected market/selected oracle metadata discovery now succeeds, selected Drift perp and spot public guardrail fields are decoded with pinned offsets, and bounded Drift liquidation-history pagination has advanced to 140,000 finalized transactions without a candidate. Jupiter Perps program/custody/oracle metadata discovery plus unverified stronger lifecycle candidate labeling now succeed. Public Jupiter proof now includes three rejected blocker fixtures: stronger unverified lifecycle, weak lifecycle, and malformed source authority. Phoenix/Rise is now source-pinned for production program and Hawkeye view planning. Jupiter canonical IDL/source confirmation, verified request/fulfillment reconstruction, Phoenix account-level decode, and Phoenix liquidation replay remain next implementation steps only after source-review validators and scrub checks pass.
