...
The Helius integration is limited to read-only RPC account fetches for decode/provenance proof. RPC URLs and API keys remain local-only and are never committed to the repo or included in public datasets. Drift program/state/selected market/selected oracle metadata discovery now succeeds, selected Drift perp and spot public guardrail fields are decoded with pinned offsets, and bounded Drift liquidation-history pagination has advanced to 288,000 finalized transactions without a candidate. Jupiter Perps program/custody/oracle metadata discovery plus unverified stronger lifecycle candidate labeling now succeed. Public Jupiter proof now includes three rejected blocker fixtures: stronger unverified lifecycle, weak lifecycle, and malformed source authority. Phoenix/Rise is now source-pinned for production program and Hawkeye view planning, with a local validator-plan contract for scrubbed fixture gates. The slot-regime benchmark package adds source-governed pre/post windows around activation slot 440208000 while explicitly blocking performance claims. Jupiter canonical IDL/source confirmation, verified request/fulfillment reconstruction, Phoenix account-level decode, and Phoenix liquidation replay remain next implementation steps only after source-review validators and scrub checks pass.
